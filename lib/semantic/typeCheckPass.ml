...
let binop_result = function
  ...
  | (Int (left_prec, _), Remainder _, Int (right_prec, _), m)
  | (Int (left_prec, _), BOr _, Int (right_prec, _), m)
  | (Int (left_prec, _), BXor _, Int (right_prec, _), m)
  | (Int (left_prec, _), BAnd _, Int (right_prec, _), m)
    -> Some (Int (max left_prec right_prec, m))
  (* (Float) Arithmetic *)
